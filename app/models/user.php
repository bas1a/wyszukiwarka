...
// Klasa 'User' odpowiada za obsługę użytkowników
class User
{
    private $db; // Prywatna zmienna do przechowywania połączenia z bazą danych
    private $table_name = "users"; // Nazwa tabeli w bazie danych
    ...
    // Funkcja, która rejestruje nowego użytkownika
    public function addNewUser($firstName, $lastName, $email, $password)
    {
        ...
        // Wykonanie zapytania
        if ($stmt->execute()) {
            error_log("Użytkownik " . $firstName . " " . $lastName . " został dodany do bazy danych.");
            return true; // Powodzenie dodania
        } else {
            error_log("Błąd dodawania użytkownika: " . implode(";", $stmt->errorInfo()));
